...
class AnnotationEditDialog(QDialog):
    ...
    def __init__(self, ann, parent=None):
        ...
        if atype == 'text':
            ...
            self.fields['x'].setDecimals(6)
            self.fields['x'].setValue(ann['x'])
            form.addRow('X:', self.fields['x'])
            self.fields['y'] = QDoubleSpinBox()
            self.fields['y'].setRange(-1e9, 1e9)
            self.fields['y'].setDecimals(6)
            ...
            row = QHBoxLayout()
            row.addWidget(self.fields['fontcolor'])
            row.addWidget(btn_fc)
            w = QWidget()
            w.setLayout(row)
            ...
            row2 = QHBoxLayout()
            row2.addWidget(self.fields['bg_color'])
            ...
            row3 = QHBoxLayout()
            row3.addWidget(self.fields['fontcolor'])
            ...
            self.fields['zoom'].setRange(0.01,5.0)
            self.fields['zoom'].setSingleStep(0.05)
...
class DataImportDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self._sheet_combo.setMinimumWidth(120)
        ...
        self._sep_custom = QLineEdit()
        self._sep_custom.setPlaceholderText('regex / char')
        self._sep_custom.setFixedWidth(80)
        self._sep_custom.setVisible(False)
        ...
        self._skip_spin.setRange(0, 1000)
        self._skip_spin.setValue(0)
        self._skip_spin.setFixedWidth(55)
        ...
        hdr_row.addWidget(self._shape_lbl)
        hdr_row.addStretch()
        ...
        # Column picker
        picker_w = QWidget()
        picker_lay = QVBoxLayout(picker_w)
        ...
        pick_hdr.addStretch()
        pick_hdr.addWidget(btn_all)
        pick_hdr.addWidget(btn_none)
        ...
        self._col_picker_lay.setSpacing(3)
        self._col_picker_lay.setContentsMargins(4,4,4,4)
    ...
    def _rebuild_col_picker(self, df):
        ...
        lbl_inc.setFixedWidth(56)
        lbl_inc.setStyleSheet('font-weight:bold;')
        lbl_orig = QLabel('Original name')
        lbl_orig.setFixedWidth(180)
        lbl_orig.setStyleSheet('font-weight:bold;')
        lbl_type = QLabel('Type')
        lbl_type.setFixedWidth(42)
        lbl_type.setStyleSheet('font-weight:bold;')
        lbl_imp  = QLabel('Import as (rename)')
        lbl_imp.setStyleSheet('font-weight:bold;')
        hdr_l.addWidget(lbl_inc)
        hdr_l.addWidget(lbl_orig)
        hdr_l.addWidget(lbl_type)
        hdr_l.addWidget(lbl_imp)
        hdr_l.addStretch()
        ...
        for col in cols:
            ...
            orig_lbl = QLabel(col)
            orig_lbl.setFixedWidth(180)
            orig_lbl.setToolTip(col)

            type_lbl = QLabel(dtype_icon)
